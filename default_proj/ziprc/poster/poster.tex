...
% Figures: live in  default_proj/ziprc/poster/figures/  (already in this repo). Upload them
%          into a ./figures/ folder in your Overleaf project:
%          calibration_auc.png, pareto_latency.png, blend_frontier.png
%          (blend_study.png is the 2-panel frontier+calibration-law version, if more space).
...
\begin{column}{\colwidth}
  ...
  \begin{block}{Part B — Adaptive Test-Time Compute: ZIP-RC-Lite \emph{(A. Kim)}}
    ...
    \begin{itemize}
      ...
      \item \textbf{allocate} samples by predicted difficulty (across prompts).
    \end{itemize}
    We measure these alone \emph{and} \textbf{blended} (allocate $\times$ prune) under a rigorous
    prompt-bootstrap $+$ held-out inference layer (column~3).
  \end{block}
...
\begin{column}{\colwidth}
  ...
  \begin{block}{Blending the levers: adaptive-K $\times$ prune \emph{(A. Kim)}}
    Composing the across-prompt allocator with within-sample pruning, under a \textbf{rigor layer}
    (prompt-level bootstrap CIs, \textbf{held-out} operating-point selection, a
    difficulty-controlled partial correlation, $+$ leakage \& faithfulness audits):
    \begin{figure}
      \centering
      \includegraphics[width=0.92\linewidth]{figures/blend_frontier.png}
      \caption{Blend frontier (squares) sits left of fixed+full (circles): \emph{same accuracy,
      less compute}. Held-out: $\sim$20--25\% compute at \emph{neutral} accuracy (4 pools).}
    \end{figure}
    \begin{itemize}
      \item \textbf{Savings \emph{super}-compound:} the blend beats the product of the two levers
        by up to $+19$pp --- prune is \emph{more} effective under adaptive allocation (every
        CI $>0$); ``allocation-agnostic'' is \emph{rejected}.
      \item \textbf{Calibration governs prune-safety} --- but pruning acts at mid-trajectory AUC
        $\approx$0.6--0.7, far below the 0.91 value-\emph{end} AUC, so even calibrated pools pay
        some accuracy. A \emph{within-pool} tendency; the clean cross-pool law is \emph{not}
        established ($r{=}0.35$, $p{=}0.36$).
      \item \textbf{The bottleneck is the \emph{problem}, not the head:} a head-swap lowering
        calibration at \emph{fixed} difficulty (same samples) did \emph{not} change prune-safety.
      \item \textbf{Rigor caught optimism:} the single-run ``Pareto free lunch $+$ accuracy
        synergy'' did \emph{not} survive held-out inference --- the honest result is the
        efficiency gain above.
    \end{itemize}
  \end{block}
  ...
  \begin{block}{Further findings}
    \begin{itemize}
      ...
      \item \textbf{Structured 3-outcome verifier --- negative but informative:} failure-mode
        diversity is \emph{policy-capability-dependent}; no single 0.5B checkpoint balances all
        three classes, so the structured head matches binary (AUC $0.867$ vs $0.865$).
      \item \textbf{Cross-policy transfer:} the head is largely \emph{policy-agnostic} --- an
        RLOO head scores SFT rollouts at AUC $0.865$ ($\approx$ the matched SFT head's $0.867$).
    \end{itemize}
  \end{block}
  ...
  \begin{block}{Conclusions}
    \begin{itemize}
      \item \textbf{(B, A. Kim):} zero-overhead introspection on a 0.5B \emph{verifiable} task
        ...
        calibration matching the original paper. \textbf{Blending} allocation $\times$ prune
        \emph{super}-compounds into a rigorously-verified $\sim$20--25\% compute saving at
        \emph{neutral} accuracy; prune-safety is governed by \emph{mid-trajectory} separability,
        set more by the \emph{problem} than the head. Throughout, a prompt-bootstrap $+$
        held-out rigor layer \textbf{corrected the optimistic single-run claims} --- the
        structured verifier and the ``free-lunch'' both proved more modest than they first looked.
